...
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/cashpay/tpl','_header') ?>
<!--#Meta:modules/cashpay/tpl/js/modinstlist.js--><?php $__tmp=array('modules/cashpay/tpl/js/modinstlist.js','','','');Context::loadFile($__tmp);unset($__tmp); ?>
<table class="x_table">
	<thead>
		<tr>
	...
	<tbody>
		<?php if($__Context->list&&count($__Context->list))foreach($__Context->list as $__Context->no=>$__Context->val){ ?><tr>
			...
			<td><a href="<?php echo getUrl('act','dispCashpayAdminInsertModInst','module_srl',$__Context->val->module_srl) ?>" class="buttonSet buttonSetting"><span><?php echo $__Context->lang->cmd_management ?></span></a></td>
...
<div class="x_clearfix">
	<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/cashpay/tpl','_page_navigation.html') ?>
	<a class="x_btn x_pull-right" href="<?php echo getUrl('act','dispCashpayAdminInsertModInst','module_srl','') ?>"><?php echo $__Context->lang->cmd_make ?></a>
</div>
<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/cashpay/tpl','_search_bar.html') ?>
<form action="./" class="x_modal" method="post" id="deleteInstance"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	...
	<input type="hidden" name="act" value="procCashpayAdminDeleteModInst" />
